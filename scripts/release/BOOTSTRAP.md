...
5. For this one-time migration only, set the protected `PRODUCTION_VERIFY_SITE_CONTRACT_MODE` environment variable to `bootstrap`. Dispatch `Deploy Approved Pages Release` from `main` with `site_contract_mode=bootstrap` plus exact `L`, `S`, `B`, `F`, `RL`, candidate compatibility `[L,S]`, and rollback compatibility `[L]` values matching the protected `github-pages` environment variables. Bootstrap accepts only an exact legacy `/release.json` 404 together with latest successful Pages build/deployment SHA `L`; it does not accept a different or malformed response.
