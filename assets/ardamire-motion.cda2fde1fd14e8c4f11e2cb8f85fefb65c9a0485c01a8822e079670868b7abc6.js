  ...
  for (const root of document.querySelectorAll('[data-ardamire-motion]')) {
    ...
    const render = () => {
      ...
      if (activeIndex === verificationIndex) {
        ...
        status.textContent = 'Verification stage active - rollout remains separately controlled';
        play.disabled = false;
        pause.disabled = true;
        return;
      }
    ...
    if (reducedMotion) {
      ...
      status.textContent = 'Static control path - human review remains required';
      play.disabled = true;
      pause.disabled = true;
