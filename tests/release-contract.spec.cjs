...
test('Singapore source-review proof preserves exact source roles, claim boundary, and release result', () => {
  ...
  assert.match(page, /A changed version cannot reuse the prior approval/i);
  assert.match(page, /Not affiliated with or endorsed by MAS/i);
  assert.match(page, /Not legal advice or a regulatory compliance determination/i);
...
test('public research and trust routes are stable, scoped, and buyer-readable', () => {
  ...
  assert.match(index, /Verified local \/ Open full view/i);
  ...
  assert.match(index, /Editorial reconstructions from public records/i);
  assert.match(index, /Auxtho was not involved in either event/i);
  assert.match(index, /finance\.gov\.au\/sites\/default\/files\/foi-25-26-084-document-1\.pdf/i);
  assert.match(index, /dewr\.gov\.au\/assuring-integrity-targeted-compliance-framework/i);
  assert.match(index, /judiciary\.uk\/wp-content\/uploads\/2025\/06\/Ayinde-v-London-Borough-of-Haringey-and-Al-Haroun-v-Qatar-National-Bank\.pdf/i);
  ...
  assert.equal((index.match(/id="contact"/g) || []).length, 1);
  assert.doesNotMatch(index, /sales-contact|sales-verify-strip/i);
